...
# Security Review Report — F0001-repository-and-engineering-foundation run 2026-09-12-855d2b93
...
- Date: 2026-09-12
- Reviewer: Security Reviewer remediation pass
...
Credential verification failures are now recorded by the API dependency boundary as a
structured `credential_verification_failed` warning. The event includes only the
internal reason code, method, path, and request correlation ID; the bearer token is not
included. The security tests assert this contract for malformed, invalid-signature,
expired, and wrong-audience paths when the live HTTP fixture is available, and always
run a pure logging-contract test.
