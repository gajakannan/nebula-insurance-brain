...
|-------|-----|---------------------------|---------------------------|
| dependency | Yes | `engine`: 0 known vulnerabilities (`pip-audit --local`, 28 packages). `neuron`: 0 known vulnerabilities (`pip-audit --local`, 60 packages). `experience`: 0 vulnerabilities at `high` level (`pnpm audit --audit-level high`, 344 total deps). | `artifacts/security/engine-pip-audit.json`, `artifacts/security/neuron-pip-audit.json`, `artifacts/security/experience-pnpm-audit.json` |
