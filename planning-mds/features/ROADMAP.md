...
This document is the working prioritization view for feature sequencing. The tables are generated fenced regions compiled from `planning-mds/kg-source/features/**` (`roadmap_section`, `roadmap_order`, `rationale`, `validation_gate`, `completion_state`); edit the shards, never the tables. The delivery sequence for this product is master blueprint section 115.3.
...
| [F0022 — Document 360 + Label Studio evidence review integration + parent/classification and reviewer authority](./F0022-document-360-and-label-studio-evidence-review/README.md) | Planned | Document 360 with Label Studio evidence review is the v0.1 human-correction loop, including parent and classification checks and reviewer authority (sections 71, 75, 111). |
...
| [F0024 — GL vertical slice](./F0024-gl-vertical-slice/README.md) | Planned | The GL vertical slice is the v0.1 end-to-end proof: policy document to each-occurrence limit with evidence, valid and recorded time, and the review path (section 86). |
| [F0025 — Endorsement bitemporal slice](./F0025-endorsement-bitemporal-slice/README.md) | Planned | The endorsement bitemporal slice proves valid-time versus recorded-time answers on a retroactive change (section 87). |
| [F0026 — v0.1 hardening + Label Studio Golden Corpus workflow + AuthX negative tests and policy parity](./F0026-v0-1-hardening-golden-corpus-and-authx-tests/README.md) | Planned | v0.1 hardening closes the release gates: the Golden Corpus workflow, AuthX negative tests and policy parity, ingestion recovery, the deletion and revocation minimum, and the frozen evaluation slice (sections 96, 115, 121). |
...
| [F0043 — Generalized review queues / governance](./F0043-generalized-review-queues-and-governance/README.md) | Planned | Review queues generalize beyond extraction correction to conflicts, merges, candidates, and promotions, with Label Studio kept for evidence-oriented adjudication and business approval kept separate (sections 75, 111.2). |
...
| [F0055 — Native reasoning](./F0055-native-reasoning/README.md) | Planned | Native ontology inference and a business rule engine reason over canonical facts with an explicit supported subset and explicit rejection of unsupported constructs (sections 45, 92, 112). |
| [F0056 — Derived dependency invalidation](./F0056-derived-dependency-invalidation/README.md) | Planned | Derived facts point at exact input versions so a retroactive change invalidates dependents automatically (sections 55, 56). |
