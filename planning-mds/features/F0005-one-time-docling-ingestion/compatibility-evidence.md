...
## Synthetic development scope — 2026-09-21

The current scope was explicitly limited to synthetic development fixtures. [The report](synthetic-comparison.json) records nine runs across the unchanged direct baseline, Graph direct, and Graph dense. Dense makes two recorded SDK requests (skeleton and fill) over 8–9 chunks. All paths retain identical native JSON with conversion forbidden. Dense passes unique-value, ambiguity, multi-region, schema, and absent-field expectations. The baseline's first-match locator is inaccurate on repeated/multi-region cases; Graph direct lacks usable item references on the longer fixture and reports unresolved evidence. No synthetic score is presented as model accuracy.

The comparison exposed a real integration problem: pinned Graph requests legacy JSON mode for dense calls, which the bounded client previously rejected. Nebula now accepts that hint while still sending and validating the supplied JSON schema. The regression test checks that the hint cannot bypass schema enforcement. Provider-specific grammar behavior remains future live qualification.

Grounding now joins coverage across native provenance regions only when every character is supported. Each region retains its own page, box, and character range. Incomplete coverage and unsupported relationship objects remain unresolved. Evidence mapper version 2 is recorded on new runs. The importer retains every binding and combines compatible selectors for review; mixed evidence remains unresolved. The Review Panel lets a reviewer choose each region and submits the entire selector set with the decision.

Focused runtime tests: **45 passed**, covering delivery, bounded inference, and review evidence. Review-package suite: **22 passed**, **99.21%** coverage. Synthetic comparison regression: **1 passed**. Frontend: **12 passed**, lint/type checks and production build passed. Full engine and neuron/benchmark type checks passed. Existing native/scanned and PostgreSQL recovery results above remain bounded proof; F0001 archives are unchanged.

The development scope is complete. Live corpus/model measurements, production resource/retention and infrastructure failure qualification, unsupported relationship semantics, and release reviewer verdicts are future work. ADR-0060 remains Proposed and the candidate remains opt-in; those release gates are not requirements to close the current synthetic development work.
